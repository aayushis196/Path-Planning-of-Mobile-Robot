...
tspan=[0 0.001];
%controller
k=[20.0; 1.0; 40.0]; % Tuning Parameters [k_x k_y k_theta]

ref_vel= [0.0; 0.0]; %[v_ref w_ref]
ref_pose=[0.0; 0.0; 0.0]; %[x_ref y_ref theta_ref ] 
err_pose=[0.0; 0.0; 0.0]; %[x_err y_err theta_err ]
...
%Control Law
vel(1,:)= ref_vel(1,:)+ k(1,:)*err_pose(1,:)/sqrt(1+err_pose(1,:)^2+err_pose(2,:)^2);
vel(2,:)= ref_vel(2,:)+ ref_vel(1,:)*k(2,:)*(err_pose(2,:)*cos(err_pose(3,:)/2)-err_pose(1,:)*sin(err_pose(3,:)/2))/sqrt(1+err_pose(1,:)^2+err_pose(2,:)^2)+k(3,:)*sin(err_pose(3,:)/2);
