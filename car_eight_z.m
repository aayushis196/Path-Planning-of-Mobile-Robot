...
tspan=[0 0.001];
%controller
k=[1.00; 0.00; 1.00]; % Tuning Parameters [k_x k_y k_theta]
...
T_e=[cos(ref_pose(3,:)) sin(ref_pose(3,:)) 0   %Transformation Matrix
     -sin(ref_pose(3,:)) cos(ref_pose(3,:)) 0
     0 0 1 ];
...
%Control Law
vel(1,:)=ref_vel(1,:) - k(1,:)*err_pose(1,:)*abs(ref_vel(1,:));
vel(2,:)=ref_vel(2,:)- k(2,:)*ref_vel(1,:)*err_pose(2,:)-k(3,:)*abs(ref_vel(1,:))*tan(err_pose(3,:));
