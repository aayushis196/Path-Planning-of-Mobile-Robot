...
delta_max=pi/6;
v_max=20.0;
Pose = [30.0; -5.0; 1.60]; % [x y theta delta]
poseDot = 0*Pose;
vel=[0.0; 0.0];      %[v w]
tspan=[0 0.001];
%controller
k=[20.0; 5.0; 40.0]; % Tuning Parameters [k_x k_y k_theta]
...
f=sin(err_pose(3,:))/err_pose(3,:);
%Control Law
vel(1,:)=ref_vel(1,:)*cos(err_pose(3,:)) + k(1,:)*err_pose(1,:);
vel(2,:)=ref_vel(2,:)+ k(3,:)*err_pose(3,:)+k(2,:)*err_pose(2,:)*ref_vel(1,:)*f;
% 
% if(vel(1,:)<-v_max)
%     vel(1,:)=-v_max;
% end
