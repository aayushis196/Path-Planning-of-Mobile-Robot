...
delta_max=pi/6;
v_max=10.0;
Pose = [42.0; -5.0; pi/2]; % [x y theta delta]
poseDot = 0*Pose;
vel=[0.0; 0.0];      %[v w]
tspan=[0 0.001];
%controller
k=[20.0; 0.3; 0.8]; % Tuning Parameters [k_x k_y k_theta]
...
S(i)=s;
%path
% for circle x=a*cos(s) and y=a*sin(s)
%v_ref= s_dot*a
%w_ref=s_dot

x(i)= 40*cos(s);
y(i)= 40*sin(s);
x_dot=-40*sin(s)*s_dot;
y_dot=40*cos(s)*s_dot;
